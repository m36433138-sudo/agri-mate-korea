...
import { Plus, Pencil, Trash2, Tractor, Search } from "lucide-react";
...
const BRANDS = ["웅진", "웅비", "제트로", "죽암", "동양", "명성", "국제", "아세아", "위캔글로벌", "기타"];
...
export default function AttachmentsCatalog() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 max-w-6xl mx-auto">
      ...
      <CatalogFormDialog open={dialogOpen} onOpenChange={setDialogOpen} editing={editing} />
    </div>
  );
}

function CatalogFormDialog({ open, onOpenChange, editing }: {
  open: boolean; onOpenChange: (v: boolean) => void; editing: CatalogItem | null;
}) {
  const { toast } = useToast();
  const qc = useQueryClient();
