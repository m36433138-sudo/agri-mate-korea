              ...
              {machineId ? (
                ...
                <>
                  ...
                  <div className="relative">
                    ...
                    {selectedCustomerId ? (
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="ghost" size="icon" className="h-9 w-9" onClick={clearCustomer}>
                          <X className="h-4 w-4" />
                        </Button>
                      </div>
                    ) : (
                  ...
                  <div className="relative">
                    <Label>2. 기계 선택 *</Label>
                    {!selectedCustomerId ? (
                      <p className="text-xs text-muted-foreground py-2">고객을 먼저 선택해주세요.</p>
                    ) : selectedMachineId ? (
                      <div className="flex items-center gap-2">
                      ...
                      <>
                        {customerMachines.length > 0 && (
                          <div className="space-y-1 mb-2">
                            <p className="text-[11px] text-muted-foreground">이 고객의 등록 기계</p>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                              {customerMachines.map((m) => (
                                <button
                                  key={m.id}
                                  onClick={() => pickMachine(m)}
                                  className="text-left p-2 rounded-md border hover:bg-accent transition-colors"
                                >
                                  <div className="flex items-center gap-2">
                                    <span className="text-sm font-medium">{m.model_name}</span>
                                    {m.machine_type && (
                                      <span className="text-[10px] text-muted-foreground">{m.machine_type}</span>
                                    )}
                                  </div>
                                  <span className="text-[11px] text-muted-foreground font-mono">{m.serial_number}</span>
                                </button>
                              ))}
                            </div>
                          </div>
                        ...
                        <div className="relative">
                          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                          <Input
                            placeholder={customerMachines.length > 0 ? "다른 기계 검색..." : "모델명, 제조번호, 기종 검색..."}
                            value={machineSearch}
